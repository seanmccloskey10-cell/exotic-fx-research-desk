...
> **Audience note.** This README is written for an AI coding agent (Claude Code) to parse and execute. The user (Bura) is a **senior FX prop-desk operator** — AI-fluent and a heavy Cowork user, but **not a developer**. He'll say things like *"Clone this and set it up for me."* He has Bloomberg at the desk; this tool is the synthesis layer over data he already has, not a replacement for it. Clarity for the agent beats prose for a human reader.
>
...
The user (Bura) is on **macOS** (Mac mini at home). He's AI-fluent and won't be intimidated by a terminal, but he's still not a developer — handle setup, config, and any code-level changes yourself via natural-language requests from him. If a prerequisite is missing, install it yourself (with his one-time consent per install command). Work through these steps **in order**. Stop and ask him for help **only** if all automated paths fail.
...
#      → macOS asks for his password via the standard GUI dialog (lock icon) — Bura enters it there
#      → the installer adds python3 to PATH automatically
#   4. Restart Terminal so PATH picks up the new python3
```

**Why .pkg over Homebrew for non-technical users:** The Homebrew bootstrap (`/bin/bash -c "$(curl ...)"`) asks for the user's password in Terminal. Bura was visibly uncomfortable typing his password into a black box where he couldn't see what he was typing. The GUI dialog feels safe (it's the same Apple-managed dialog he sees when installing any signed app); the Terminal password prompt does not. Even though both are technically equivalent, the trust gap is real.

**Homebrew is the better option for technical users** (Sean) or once Bura has installed Homebrew once via the GUI bootstrap. If Homebrew is already present:
...
2. **TextEdit's rich-text default.** If the agent needs to edit `.env` by hand, don't open it in TextEdit — by default TextEdit saves `.rtf`, not plain text. Use VS Code (`code .env`) or `nano .env` in Terminal instead. Better: the agent should edit `.env` directly via its file-editing tools, never hand-off to Bura.
...
4. **Homebrew is the default package manager.** Most open-source Mac tools assume Homebrew is installed. If Bura doesn't have it, install it (step 1) — it's a one-time setup that makes every future install trivial.
...
Everyone: [PRD.md](PRD.md) is the full spec. [APIS.md](APIS.md) is the menu of additional data sources Bura can add.
